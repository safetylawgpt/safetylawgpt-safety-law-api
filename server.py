...
def nfkc(s: str) -> str:
    # 완전일치 판단 시 유니코드 정규화 + 좌우 공백 제거
    return unicodedata.normalize("NFKC", (s or "")).strip()

def as_int(date_str: str) -> int:
    # YYYYMMDD -> 정렬용 정수, 없으면 0
...
# 쿼리 파라미터:
#   keyword=검색어(필수)
#   exact=true/false (기본 true: 법령명 100% 일치만 통과)
@app.get("/search")
def search_law():
    keyword = request.args.get("keyword", "").strip()
    exact = request.args.get("exact", "true").lower() != "false"  # default: True
    ...
    try:
        ...
        for law in root.findall("law"):
            ...
            # --- [1] 법령명 완전일치 필터 ---
            # exact=true 일 경우: 질문 keyword와 법령명(정규화)이 100% 동일해야 통과
            if exact and nfkc(law_name) != kw_norm:
                continue
        ...
        # --- [2] 최신 시행일 우선 정렬(내림차순) ---
        laws.sort(key=lambda x: as_int(x.get("시행일자")), reverse=True)

        if STRICT_MODE and not laws:
            return hard_fail("정확한 조문을 찾을 수 없습니다. 국가법령정보센터에서 직접 확인하십시오.")
